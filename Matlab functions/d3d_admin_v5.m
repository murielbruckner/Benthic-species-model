...
% Author Mijke van Oorschot, adjusted by Muriel Br?ckner 23/4/2021
% Function to handle administration in Delft3D when coupling the benthos
...
% adjusted to this value and for each timestep results are written in the
% results folder
% ets = running ID for the ecological loop
% mdf = the old mdf file
% year = running ID for the year loop
% m  = if morphology is taken into account
% res = restart option activated
% Tref = is the initial start time when restart option is activated

% adjust mdf- and mor-file
if year ==1 && ets ==1 % after first timestep MB_0: initial conditions have to be deleted

% delete initial conditions for first ets if no restart 
try 
